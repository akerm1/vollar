...
function cleanupHourlyBackups(){
  ...
  var KEEP_NEWEST=(typeof _backupKeepCount==='number')?_backupKeepCount:7;
...
function setHourlyExportFromSettings(enabled,minutes,retention,path){
  ...
  if(retention)_hourlyExportRetention=parseInt(retention)||60;
...
var _backupKeepCount = 7; // see cleanupHourlyBackups (keep more recovery history)
var _lastAutoBackupMs = 0;      // per-sale snapshot throttle
var _autoBackupMinGapMs = 60000; // min gap between automatic full snapshots

// Prune the app's own backup_*/export_* .json files left in the OS Downloads
// folder (the no-folder fallback writes there). Keeps the newest 30. Desktop only.
function cleanupDownloadsBackups() {
  var a = _vapp();
  if (!a || !a.getDownloadsPath || !a.listFiles || !a.deleteFile) return Promise.resolve();
  return a.getDownloadsPath().then(function (r) {
    if (!r || !r.path) return;
    return a.listFiles(r.path).then(function (res) {
      if (!res || !res.files) return;
      var re = /^(backup_|export_)[A-Za-z0-9._-]+\.json$/;
      var ours = res.files.filter(function (f) { return f.name && re.test(f.name); });
      ours.sort(function (x, y) { return (y.mtime || 0) - (x.mtime || 0); });
      ours.slice(30).forEach(function (f) {
        a.deleteFile(r.path + '/' + f.name).catch(function () {});
      });
    });
  }).catch(function () {});
}

// Keep only the last 180 days of audit-log entries; pruned at startup.
function pruneAuditLog() {
  var DAY = 24 * 60 * 60 * 1000;
  var cutoff = Date.now() - 180 * DAY;
  return dbGetAll('auditlog').then(function (rows) {
    var stale = rows.filter(function (r) {
      return r && r.timestamp && new Date(r.timestamp).getTime() < cutoff;
    });
    if (!stale.length) return;
    var ops = stale.map(function (r) { return { store: 'auditlog', op: 'delete', key: r.id }; });
    return window['dbMultiOp'](ops).then(function () {
      console.log('Audit log pruned: removed ' + stale.length + ' entries older than 180 days.');
    }).catch(function (err) { console.error('Audit log prune failed:', err); });
  }).catch(function () {});
}
...
(function () {
  ...
  window['downloadBackupSilent'] = function (name, content) {
    ...
    var result = origDownloadBackupSilent(name, content);
...
// Housekeeping once per session, right after the DB is open (settings are
// applied during bootstrap, which is the first call to setHourlyExportFromSettings).
(function () {
  var done = false;
  var orig = window['setHourlyExportFromSettings'];
  window['setHourlyExportFromSettings'] = function (enabled, minutes, retention, path) {
    if (!done) {
      done = true;
      if (settings && settings.hourlyExportRetention === 30) {
        settings.hourlyExportRetention = 60;
        dbPut('settings', { key: 'hourlyExportRetention', value: 60 });
      }
      setTimeout(function () {
        pruneAuditLog();
      }, 1500);
...
(function () {
  ...
  if (typeof origManual === 'function') {
    window['manualBackup'] = function () {
      forceNext = true;
      var p = origManual.apply(this, arguments);
      if (p && typeof p.then === 'function') {
        p.then(
          function (r) { forceNext = false; return r; },
          function (e) { forceNext = false; throw e; }
        );
      } else {
        forceNext = false;
      }
