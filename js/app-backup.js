...
function cleanupHourlyBackups(){
  ...
  a.listFiles(_hourlyExportPath).then(function(result){
    ...
    result.files.forEach(function(f){
      if(f.name&&f.name.indexOf('backup_')===0&&f.mtime<cutoff){a.deleteFile(_hourlyExportPath+'/'+f.name)['catch'](function(){});}
    });
  })['catch'](function(){});
}
