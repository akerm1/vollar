...
- **Per-client updates** (Settings > Système, "Mise à jour de l'application"): a master switch `settings.updateCheckEnabled` (default `true`) gates EVERY check on that poste (main returns `'disabled'`), and the GitHub source is editable in-app — `settings.updateRepoOwner`/`settings.updateRepo` are pushed to the main process via the `update-source` IPC, which calls `autoUpdater.setFeedURL({provider:'github', owner, repo})` (empty fields keep the feed baked from `build.publish`). A build can bake per-client defaults without touching settings by setting env vars before `npm run dist`: `SAMTEX_UPDATES_DISABLED=1`, `SAMTEX_GITHUB_OWNER=...`, `SAMTEX_GITHUB_REPO=...` — `build-release.js` (`applyUpdateDefaults`) injects `window.__updateDefaults` into the staged index.html. The parcel "Sauvegarde" pane (index.html `#panel-backup`) shows the effective backup destination (`#backup-dest-display`, refreshed by `updateBackupDestDisplay()` in `js/bootstrap/app-backup.js`).
...
- **Settings** (`js/settings/`): `settings.js`, `scanner-settings.js` (panel Scanner + test de scan), `backup-restore.js` (panneau « ♻️ Restaurer une sauvegarde » : liste les `backup_*.json` du dossier d'export + Bureau, restauration en un clic via `doSafeImport`), `exports.js`, `keyboard.js`, `themes-data.js`/`themes.js`. (Auto-backup lives in `js/core/backup.js` + `js/bootstrap/app-backup.js`.)
